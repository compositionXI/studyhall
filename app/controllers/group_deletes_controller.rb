class GroupDeletesController < ApplicationController
  ...
  def new
    @group_delete = GroupDelete.new(class_name: "study_session")
  end

  def create
    @group_delete = GroupDelete.new(params[:group_delete])
    if @group_delete.process
      flash[:notice] = "Deleted #{pluralize(@group_delete.object_ids_array.size,"StudyHall")}"
    else
      flash[:warning] = "Uh-oh! There was a problem. Please try again."
    end
    redirect_to send(@group_delete.class_name+"s_path")
  end
