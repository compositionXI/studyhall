...
(function() {
  $(function() {
    var newStudyhallModal;
    newStudyhallModal = $("#new_studyhall_modal");
    newStudyhallModal.dialog({
      modal: true,
      autoOpen: false
    });
    return $("#new_sutdyhall_btn").click(function(e) {
      return $.get("/study_sessions/new", function(data) {
        newStudyhallModal.html(data);
        newStudyhallModal.dialog('open');
        return newStudyhallModal.find(".close_modal").click(function(e) {
          e.preventDefault();
          return newStudyhallModal.dialog('close');
        });
      });
    });
  });
...
(function() {
  $(function() {
    return $(".masonry_container").masonry({
...
;
(function() {
  var NotebookContainer, NotebooksController, newNotebookModal;
  NotebookContainer = (function() {
    var HASH_PREFIX, className;
    HASH_PREFIX = '#layout';
    className = 'notebook_list_container';
    function NotebookContainer(theClassName) {
      className || (className = theClassName);
      this.container = $('.' + className);
    }
    NotebookContainer.prototype.setup = function() {
      this.setupExpandCollapseIcon();
      this.setupDragNDrop(this.container);
      return this.setupSelectable();
    };
    NotebookContainer.prototype.setupExpandCollapseIcon = function() {
      return this.container.delegate('a.expand_collapse_icon', 'click', function() {
    ...
    NotebookContainer.prototype.setupDragNDrop = function(scope) {
      var notebookContainer, notebookList;
      notebookList = $('.notebook_list', this.container);
      notebookContainer = this;
      $('.note_list_item', scope).draggable({
        start: function(e, ui) {
          return $(this).addClass('dragging_placeholder');
        },
        stop: function(e, ui) {
          return $(this).removeClass('dragging_placeholder');
        },
        helper: function() {
          var theHelper;
          theHelper = $(this).clone().appendTo(notebookList);
          return theHelper.width($(this).width());
        },
        handle: '.drag_handle',
        scope: 'notes',
        revert: 'invalid',
      ...
      return $('.notebook', scope).droppable({
        scope: 'notes',
        hoverClass: 'draggable_hovering',
        drop: function(e, ui) {
          return notebookContainer.moveNoteToNotebook($('.note', ui.draggable), $(this));
        }
      });
    };
    NotebookContainer.prototype.setupSelectable = function() {
      this.container.delegate('.preview', 'click', function(event) {
        ...
        if (!$(event.target).is('a')) {
          if (checkbox.prop('checked')) {
            ...
            return container.removeClass('checked');
          } else {
            checkbox.prop('checked', true);
            return container.addClass('checked');
          }
        }
      });
      return this.renderSelectable();
    };
    NotebookContainer.prototype.renderSelectable = function() {
      return $('.note, .notebook').each(function() {
        var checkbox;
    ...
    NotebookContainer.prototype.moveNoteToNotebook = function(note, notebook) {
      var noteId, notebookContainer, notebookId;
      notebookContainer = this;
      ...
      return $.ajax({
        ...
        success: function(data) {
          var newNotebookItem, oldNoteList, oldNotebook;
          ...
          if ($('li', oldNoteList).length === 0) {
            ...
            return $('.expand_collapse_icon', oldNotebook).remove();
          }
        },
        error: function() {
          return alert('Cannot move note to the notebook');
        }
      });
    };
    NotebookContainer.prototype.useLayout = function(layout) {
      this.container.attr('class', [className, layout].join(' '));
      return window.location.hash = [HASH_PREFIX, layout].join('/');
    };
    NotebookContainer.prototype.recoverLayout = function() {
      var layout, prefix, _ref;
      _ref = location.hash.split('/', 2), prefix = _ref[0], layout = _ref[1];
  ...
  NotebooksController = (function() {
    function NotebooksController() {}
    NotebooksController.prototype.setup = function() {
      this.setupModalDialog();
      return this.setupNotebookContainer();
    };
    NotebooksController.prototype.setupModalDialog = function() {
      $("#new_notebook_modal").dialog({
        modal: true,
        autoOpen: false
      });
      return $("#new_notebook_btn").click(function(e) {
        return $.get("notebooks/new", function(data) {
          var newNotebookModal;
          newNotebookModal = $("#new_notebook_modal");
          newNotebookModal.html(data);
          newNotebookModal.dialog('open');
          return newNotebookModal.find(".close_modal").click(function(e) {
            return newNotebookModal.dialog('close');
          });
        });
      });
    };
    NotebooksController.prototype.setupNotebookContainer = function() {
      var notebookContainer;
      notebookContainer = new NotebookContainer();
      notebookContainer.setup();
      notebookContainer.recoverLayout();
      return $("#items_layout_switcher").delegate('a[data-layout]', 'click', function() {
  ...
  $(function() {
    return new NotebooksController().setup();
  });
  newNotebookModal = $("#new_notebook_modal");
  ...
  $("#new_notebook_btn").click(function(e) {
    $.get("notebooks/new", function(data) {
      newNotebookModal.html(data);
      return newNotebookModal.dialog('open');
    });
    return newNotebookModal.find(".close_modal").click(function(e) {
      return newNotebookModal.dialog('close');
    });
...
(function() {
  $(document).ready(function() {
    var note_content, saveNoteModal;
    ...
    $("#save_note_btn").click(function(e) {
      return saveNoteModal.dialog('open');
    });
    return $("#save_note_submit_button").click(function(e) {
      note_content = $("#rte_area_note_content").contents().find("body").html();
      return $("#note_content_hidden_field").attr("value", note_content);
    });
  });
}).call(this);
(function() {
...
(function() {
  var switch_greek_select_box;
  switch_greek_select_box = function() {
    if ($('.gender select').attr('value') === "Male") {
      $('.fraternity').css("display", "block");
      return $('.sorority').css("display", "none");
    } else {
      $('.sorority').css("display", "block");
      return $('.fraternity').css("display", "none");
    }
  };
  $(function() {
    var change_profile_photo_modal, editModals, edit_gpa_modal, edit_personal_info_modal, edit_profile_photo_modal, photoSuccess;
    $("#profile_tabs").tabs();
    $(".chzn-select").css({
      "display": "none"
    });
    $(".chzn-select").chosen();
    $('#user_extracurriculars_chzn.chzn-container .chzn-choices .search-field').change(function() {
      $('.chzn-select.appendable').append('<option>' + this.value + '</option>');
      return $('.chzn-select.appendable').trigger('liszt:updated');
    });
    $('.sorority, .fraternity').css("display", "none");
    switch_greek_select_box();
    $('.gender select').change(function() {
      return switch_greek_select_box();
    });
    ...
    $(".profile_edit_modal").dialog({
      ...
      open: function(event, ui) {
        return $(".ui-dialog-titlebar-close", ui.dialog).hide();
      }
    });
    $("#profile_personal_info").click(function(e) {
      return edit_personal_info_modal.dialog("open");
    });
    $("#profile_edit_gpa").click(function(e) {
      return edit_gpa_modal.dialog("open");
    });
    $("#change_profile_photo").click(function(e) {
      return change_profile_photo_modal.dialog("open");
    });
    $(".close_modal").click(function(e) {
      return $(this).parents(".profile_edit_modal").dialog("close");
    });
    edit_personal_info_modal = $(".edit_personal_info");
    edit_gpa_modal = $(".edit_gpa");
    ...
    $(editModals).find(".save").click(function() {
      var modal;
      modal = $(this).parents(".profile_edit_modal");
      return modal.dialog("close");
    });
    photoSuccess = function(result) {
      edit_profile_photo_modal.dialog("close");
      return $("#profile_sidebar #profile_photo").attr("src", result.avatar_url);
    };
    $("#edit_profile_photo_form").bind("ajax:success", photoSuccess);
    $("#edit_personal_info_form").bind("ajax:success", function(event, response) {
      return $("#profile_personal_info").animate({
        opacity: 0
      }, 200, function() {
    ...
    $("#edit_gpa_form").bind("ajax:success", function(event, response) {
      return $("#profile_edit_gpa").animate({
        opacity: 0
      }, 200, function() {
    ...
    return $("#extracurriculars").bind("keydown", function(event) {
      if ((event.keyCode === $.ui.keyCode.TAB) && $(this).data("autocomplete").menu.active) {
        return event.preventDefault();
      }
    }).autocomplete({
      source: function(response) {
        return $.getJSON("extracurriculars", {}, response);
      }
    });
  });
}).call(this);
