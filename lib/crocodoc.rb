...
#
  ...
  #end
#
  #def self.upload(token, document)
    #params = {"token" => token, "async" => "false"}
    #if document.kind_of? String
      #params["url"] = document
    #elsif document.kind_of? Paperclip::Attachment
      #params["file"] = document.file
    #else
      #raise ArgumentError.new("#{document.class} is invalid. You must supply a file or uri pointing to a file.") if document.nil?
    #end
    #Rails.logger.info(params.inspect)
    #response = self.get("/api/v1/document/upload?", :body => params)
    #raise response.inspect
    #raise UploadError.new("There was a problem uploading the document: #{response["error"]}") if !response["error"].nil?
    #response
  #end
#
...
#
  #def check_for_token
    #raise MissingTokenError("Whoa buddy, where's your API token?") if @token.nil?
  #end
#end
#
